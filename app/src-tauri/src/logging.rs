...
pub fn log_path() -> PathBuf {
    ...
    std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("claude-code-gui.log")))
        .unwrap_or_else(|| std::env::temp_dir().join("claude-code-gui.log"))
}
...
pub fn init() {
    ...
    match file {
        Ok(mut f) => {
            let _ = writeln!(f, "[{}] === Claude Code GUI started ===", timestamp());
