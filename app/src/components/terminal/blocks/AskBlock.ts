...
import { fg, BOLD, DIM, RESET } from "../AnsiUtils";
  ...
  render(_cols: number, palette: TerminalPalette): string {
    ...
    if (this.resolved && this.answers) {
      for (let i = 0; i < this.questions.length; i++) {
        ...
        lines.push(`  ${fg(palette.textDim)}${q.header}:${RESET} ${a}`);
      }
      return lines.join("\r\n") + "\r\n";
    }
    ...
    if (!q) {
      return `  ${DIM}(waiting for answer...)${RESET}\r\n`;
    }

    lines.push(`  ${BOLD}${q.question}${RESET}`);
    if (q.header) {
      lines.push(`  ${fg(palette.textDim)}${q.header}${RESET}`);
    }
    lines.push("");

    for (let i = 0; i < q.options.length; i++) {
      const opt = q.options[i];
      lines.push(`  ${fg(palette.accent)}${i + 1}.${RESET} ${opt.label}`);
      if (opt.description) {
    ...
    if (this.questions.length > 1) {
      lines.push(`  ${DIM}(${this.currentStep + 1}/${this.questions.length})${RESET}`);
    }
